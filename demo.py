...
def get_and_process_data(data_dir):
    ...
    color = np.array(Image.open('color_t.png'), dtype=np.float32) / 255.0
    depth = np.array(Image.open('depth_t.png'), dtype=np.float32)
    print(depth.max(), depth.min())
    # depth = np.memmap('doc/demo/01_Color_Depth.raw', dtype='uint16', mode='r', shape=(480, 640))
