...
def process_grasp_labels(end_points):
    ...
    clouds = end_points['point_clouds'] #(B, N, 3)
    seed_xyzs = end_points['fp2_xyz'] #(B, Ns, 3)
    batch_size, num_samples, _ = seed_xyzs.size()
    
    indices = end_points['indices'] #(B, Ns, 3)
    voxel_pc = end_points['voxel_pc'] #(B, H, W, L)
    B, Ns, _ = indices.shape
    batch_indices = torch.arange(B).view(B, 1).expand(B, Ns)  # (B, Ns)
    h_indices = indices[..., 0]  # H索引 (B, Ns)
    w_indices = indices[..., 1]  # W索引 (B, Ns)
    l_indices = indices[..., 2]  # L索引 (B, Ns)
    voxel_values = voxel_pc[batch_indices, h_indices, w_indices, l_indices]  # (B, Ns)
    ...
    end_points['batch_voxel_label'] = voxel_values
...
def assign_euler_angle_labels(end_points):
    """ Assign euler angle labels to views labels. """
    euler_inds = end_points['euler_inds'] # (B, Ns, 3)  # TODO: Assign ZYX to Euler from the voxelization
    euler_inds_score = end_points['euler_inds_score'] # (B, Ns, 3)
    
    template_views = generate_grasp_views(300).to(euler_inds.device) # (num_view, 3)
    B, Ns, _ = euler_inds.size()
