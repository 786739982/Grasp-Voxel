...
def get_loss(end_points):
    ...
    voxel_loss, end_points = compute_voxel_loss(end_points)
    loss = objectness_loss + view_loss + 0.2 * grasp_loss + voxel_loss
    end_points['loss/overall_loss'] = loss
    return loss, end_points

def  compute_voxel_loss(end_points):
    voxel_score = end_points['voxel_score']
    voxel_label = end_points['batch_voxel_label']
    pos_weight = 2.0
    criterion = nn.BCELoss(reduction='mean', weight=voxel_label * (pos_weight - 1) + 1)
    loss = criterion(voxel_score, voxel_label)  # 计算损失
    loss = torch.min(loss, torch.tensor(1).cuda())
    loss = loss * 0.3
    end_points['loss/stage1_voxel_loss'] = loss
