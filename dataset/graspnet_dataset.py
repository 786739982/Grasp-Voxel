...
class GraspNetDataset(Dataset):
    def __init__(self, root, valid_obj_idxs, grasp_labels, camera='kinect', split='train', num_points=20000,
                 remove_outlier=False, remove_invisible=True, augment=False, load_label=True, geom=None):
    ...
    def get_data_label(self, index):
        ...
        voxel_pc, indices_maps = voxelization(cloud_sampled, self.geom, center, training=True)
        # devoxel_pc = devoxelization(voxel_pc, self.geom, center)
        # np.save('voxel_pc.npy', voxel_pc)
        # np.save('cloud_sampled.npy', cloud_sampled)
        # np.save('devoxel_pc.npy', devoxel_pc)
        ...
        ret_dict['voxel_pc'] = voxel_pc
        ret_dict['center'] = np.array(center)
        ret_dict['indices_maps'] = indices_maps

        return ret_dict

def load_grasp_labels(root):
    obj_names = list(range(1))
    valid_obj_idxs = []
    grasp_labels = {}
